...
def build_model():
    ...
    x = layers.Conv2D(128, (3, 3), activation='relu')(inputs)  #1
    x = layers.MaxPool2D((2, 2))(x)                            #2
    x = layers.Dropout(0.25)(x)
    x = layers.Conv2D(256, (3, 3), activation='relu')(x)       #3
    x = layers.MaxPool2D((2, 2))(x)                            #4
    x = layers.Dropout(0.25)(x)
    x = layers.Conv2D(384, (3, 3), activation='relu')(x)       #5
    x = layers.MaxPool2D((3, 3))(x)
    x = layers.Dense(256)(x)                                   #7
    x = layers.Dense(128)(x)                                   #8
    x = layers.Dropout(0.25)(x)
